
#ifndef _TFT_DRV_H
#define _TFT_DRV_H
#include "platform.h"

#define USE_HORIZONTAL 1


#if USE_HORIZONTAL==1
	#define LCD_W 240
	#define LCD_H 320

#else
	#define LCD_W 320
	#define LCD_H 240
#endif



#define TFT_CLOCK_PIN    GPIO_PIN_3
#define TFT_CLOCK_PORT   GPIOB

#define TFT_MISO_PIN  GPIO_PIN_5
#define TFT_MISO_PORT GPIOB

#define TFT_MOSI_PIN  GPIO_PIN_4
#define TFT_MOSI_PORT GPIOB

#define TFT_CS_PIN    GPIO_PIN_8
#define TFT_CS_PORT   GPIOB

#define TFT_RST_PIN  GPIO_PIN_7
#define TFT_RST_PORT GPIOB

#define TFT_DC_PIN  GPIO_PIN_6
#define TFT_DC_PORT GPIOB

#define TFT_CS_LOW()  HAL_GPIO_WritePin(TFT_CS_PORT, TFT_CS_PIN, GPIO_PIN_RESET)
#define TFT_CS_HIGH()  HAL_GPIO_WritePin(TFT_CS_PORT, TFT_CS_PIN, GPIO_PIN_SET)
#define TFT_RS_CMD()   HAL_GPIO_WritePin(TFT_DC_PORT,TFT_DC_PIN,GPIO_PIN_RESET)//DC
#define TFT_RS_DATA()   HAL_GPIO_WritePin(TFT_DC_PORT,TFT_DC_PIN,GPIO_PIN_SET)
#define TFT_RES_Clr()  HAL_GPIO_WritePin(TFT_RST_PORT,TFT_RST_PIN,GPIO_PIN_RESET)//RES
#define TFT_RES_Set()  HAL_GPIO_WritePin(TFT_RST_PORT,TFT_RST_PIN,GPIO_PIN_SET)

void lcd_init(void);
void LCD_Fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t color);
void LCD_DrawPoint(uint16_t x, uint16_t y, uint16_t color);
void LCD_Clear(uint16_t Color);
#endif /*__ pinoutConfig_H */


